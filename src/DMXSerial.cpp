...
//uint8_t  _dmxData[DMXSERIAL_MAX+1]; ///No huge static buffer anymore
uint8_t * _dmxData = NULL;
...
void DMXSerialClass::init(int mode)
{
  ...
  // (Re)Initialize the specified mode.
// The mode parameter should be a value from enum DMXMode.
void DMXSerialClass::init (int mode, int dmxModePin, int channels)
{
#ifdef SCOPEDEBUG
  ...
  if (channels > DMXSERIAL_MAX) channels = DMXSERIAL_MAX;
  
  _dmxData  = new uint8_t [channels+1];
  
  _dmxDataPtr = _dmxData;
  _dmxDataLastPtr = _dmxData + channels; 
...
//  memset(_dmxData, 0, sizeof(_dmxData));
  ...
  if (_dmxMode == DMXController) {
    ...
    _dmxMaxChannel = channels; // The default in Controller mode is sending 32 channels.
    ...
    _dmxMaxChannel = channels; // The default in Receiver mode is reading all possible 512 channels.
